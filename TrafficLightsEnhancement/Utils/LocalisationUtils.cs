...

{
    ...
    private static Dictionary<string, string> m_DefaultDictionary;
    ...
    private static void EnsureDefaultDictionary()
    {
        if (m_DefaultDictionary != null)
        {
            return;
        }

        string resourceName = "C2VM.TrafficLightsEnhancement.Resources.Localisations." + m_DefaultLocale + ".json";
        using Stream stream = Assembly.GetExecutingAssembly().GetManifestResourceStream(resourceName);
        if (stream == null)
        {
            Mod.m_Log.Error($"{resourceName} does not exist.");
        ...
        using StreamReader reader = new StreamReader(stream, System.Text.Encoding.UTF8);
        string jsonString = reader.ReadToEnd();
        m_DefaultDictionary = JsonConvert.DeserializeObject<Dictionary<string, string>>(jsonString);
    }
...

    {
        ...
        if (m_DefaultDictionary != null && m_DefaultDictionary.ContainsKey(key))

        {

            return m_DefaultDictionary[key];
...

    {
        ...
        dictionary.Add(Mod.m_Settings.GetOptionLabelLocaleID("m_DefaultAdvancedSplitPhasing"), this.GetString("DefaultAdvancedSplitPhasingLabel"));

        dictionary.Add(Mod.m_Settings.GetOptionLabelLocaleID("m_ForceNodeUpdate"), this.GetString("ForceAllNodesUpdateLabel"));

        dictionary.Add(Mod.m_Settings.GetOptionLabelLocaleID("m_ResetAgents"), this.GetString("ResetAgentsLabel"));
        ...
        dictionary.Add(Mod.m_Settings.GetOptionDescLocaleID("m_DefaultAdvancedSplitPhasing"), this.GetString("DefaultAdvancedSplitPhasingDesc"));

        dictionary.Add(Mod.m_Settings.GetOptionDescLocaleID("m_ForceNodeUpdate"), this.GetString("ForceAllNodesUpdateDesc"));

        dictionary.Add(Mod.m_Settings.GetOptionDescLocaleID("m_ResetAgents"), this.GetString("ResetAgentsDesc"));
        ...
        dictionary.Add(Mod.m_Settings.GetOptionWarningLocaleID("m_ResetAgents"), this.GetString("ResetAgentsWarning"));
